...
| Human-in-the-loop | `interrupt()`, <Syntax py="interrupt_before" ts="interruptBefore" /> | interrupts raised from a hook or a tool |
...
<Syntax py="execution_order" ts="the results array" />, so you can assert the
...
:::note[Parameter descriptions reach the model differently]
<Tabs>
<Tab label="Python">
...
yourself and pass them in with `Agent(messages=...)` when `run_case` rebuilds the
graph.
...
the result's node list (<Syntax py="execution_order" ts="the results array" />),
